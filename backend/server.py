...
def _build_pdf_artifact(
    ...
    generated_from_role: str = "assistant",
    images: Optional[List[ChatImage]] = None,
) -> ChatArtifact:
    """Generate a lightweight PDF artifact with text plus any retrieved images."""
    try:
        import fitz
    ...
    available_text_height = footer_y - (margin_top + 64)
    ...
    while current_index < len(wrapped_lines) or page_number == 1:
        ...
        while current_index < len(wrapped_lines):
            next_line = wrapped_lines[current_index]
            if y + line_height > footer_y - 110:
                break
            if next_line:
                page.insert_text((margin_x, y), next_line, fontsize=body_font_size, fontname="helv", fill=(0.133, 0.192, 0.31))
            y += line_height
            current_index += 1

        while image_index < len(image_entries):
            image_item = image_entries[image_index]
            image_url = (image_item.url or "").strip()
            if not image_url:
                image_index += 1
                continue
            try:
                image_response = httpx.get(image_url, timeout=20.0, follow_redirects=True)
                image_response.raise_for_status()
                image_bytes = image_response.content
            except Exception as exc:
                logger.warning("PDF image download failed for %s: %s", image_url, exc)
                image_index += 1
                continue

            try:
                image_document = fitz.open(stream=image_bytes, filetype="png")
                ...
                try:
                    ...
                    logger.warning("PDF image decode failed for %s: %s", image_url, exc)
                    image_index += 1
                    continue

            caption = _truncate_title(image_item.source_title or image_item.alt or "Retrieved image", limit=72)
            caption_lines = textwrap.wrap(caption, width=72) or [caption]
            caption_height = max(20, len(caption_lines) * 14)
            max_width = page_width - (margin_x * 2)
            max_height = max(120, available_text_height * 0.38)
            width_scale = max_width / max(image_rect.width, 1)
            height_scale = max_height / max(image_rect.height, 1)
            scale = min(width_scale, height_scale, 1.0)
            render_width = max(120, image_rect.width * scale)
            render_height = max(90, image_rect.height * scale)

            needed_height = render_height + caption_height + 28
            if y + needed_height > footer_y:
                break

            image_box = fitz.Rect(margin_x, y, margin_x + render_width, y + render_height)
            page.insert_image(image_box, stream=image_bytes, keep_proportion=True)
            caption_y = image_box.y1 + 16
        ...
        page.insert_text(
            (page_width - margin_x - 42, footer_y),
            str(page_number),
            fontsize=10,
            fontname="helv",
            fill=(0.44, 0.5, 0.64),
        ...
        if current_index >= len(wrapped_lines) and image_index >= len(image_entries):
            break
...
def _resolve_pdf_export_source(message: str, conversation_history: Optional[List[Dict]]) -> Optional[Dict[str, Any]]:
...
def _build_pdf_chat_result(source_message: Dict[str, str]) -> dict:
    ...
    artifact = _build_pdf_artifact(
        content,
        title=f"{title_prefix} PDF",
        generated_from_role=role,
        images=source_message.get("images", []) or [],
    )
    return {
        "response": (
